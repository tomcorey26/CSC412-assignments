...
void sortString(char *s)
{
    ...
    for (int i = 0; s[i] != '\0'; i++)
    {
        if (!isReapeat(repeatStr, s[i]))
        {
            ...
            else if (isInRange(s[i], math.ranges[0]) || isInRange(s[i], math.ranges[1]))
            {
                mathCount++;
            }
    ...
    char *mathStr = (char *)malloc(mathCount + 1);
    ...
    for (int i = 0; repeatStr[i] != '\0'; i++)
    {
        ...
        else if (isInRange(repeatStr[i], math.ranges[0]) || isInRange(repeatStr[i], math.ranges[1]))
        {
            mathStr[mathCount - 1] = repeatStr[i];
            mathCount--;
        }
        else
        {
